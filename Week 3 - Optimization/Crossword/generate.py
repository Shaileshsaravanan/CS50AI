...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap is not None:
            ...
            for value_x in self.domains[x].copy():
                if not any(value_x[i] == value_y[j] for value_y in self.domains[y]):
    ...
    def consistent(self, assignment):
        ...
        for var, value in assignment.items():
            if var.length != len(value) or value in words_set:
                return False
            words_set.add(value)
    ...
    def order_domain_values(self, var, assignment):
        ...
        def count_conflicts(value):
            ...
            for neighbor in self.crossword.neighbors(var) - set(assignment.keys()):
                i, j = self.crossword.overlaps[var, neighbor]
                count += sum(1 for neighbor_value in self.domains[neighbor] if value[i] != neighbor_value[j])
            return count
